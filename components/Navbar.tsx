...
export default function Navbar() {
  ...
  return (
    <nav className={ `duration-700 text-center uppercase flex items-center justify-between flex-wrap bg-white py-1 px-7 xl:px-64 max-w-none z-20 fixed w-full z-40 ${
      collapse && "opacity-70"
      }`}>
      <div className="flex items-center flex-shrink-0 text-white">
        <Imagotipo className="w-40 h-20 hidden xl:block normal-case" alt="logo" />
      ...
      <div className="hidden xl:block flex text-sm mx-auto items-center 2xl:pr-36">
        ...
        <Link href="/servicios">
          <a className={ `block mt-4 xl:inline-block xl:mt-0 hover:text-orange-dark mx-5 uppercase ${
      ...
      {/* Mobile */}
      <div className={collapse ? 'hidden w-full flex-grow' : 'block xl:hidden w-full flex-grow'} >
        <div className="text-sm">
          <Link href="/">
            <a className={`block mt-4 hover:text-orange-dark uppercase ${
            router.asPath === "/"
              ? "text-orange-default"
              : "text-black"
            }`}>
